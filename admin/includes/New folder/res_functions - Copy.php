...
function ResTagsSave (){  // create relationship between resource and tags
    global $conn, $inserted_resource_id, $tags;
...
function ResTopicsSave(){
        global $conn, $inserted_resource_id, $topics_id;
...
function ResTypesSave (){
    global $conn, $inserted_resource_id, $restypes;
...
function ResLevelsSave (){
    global $conn, $inserted_resource_id, $rlevels;
...
function createResource($request_values) {
   global $conn, $errors, $r_title, $r_iconname, $inserted_resource_id, $topics_id, $r_info, $source, $restypes, $link_names, $link_urls, $subpage_show, $rlevels, $tags,$mediatype;
    ...
    if (empty($topics_id) ) {
        array_push($errors, "Atleast a topic is required for your resource and must be number");
    }
    if (empty($link_urls)) {
     ...
     if (count($errors) == 0) {
      ...
      //   $subpage_show = filter_var($subpage_show, FILTER_SANITIZE_BOOLEAN); 
        ...
        if ($r_inserted) {
            ...
            // create relationship between resource and school levels
            for ($k = 0; $k < count($rlevels); $k++) {
                $rlevel = filter_var(esc($rlevels[$k]), FILTER_SANITIZE_NUMBER_INT);
                $rlevel_sql = "INSERT INTO resource_level (resource_id, schl_id) VALUES($inserted_resource_id, $rlevel)";
                mysqli_query($conn, $rlevel_sql);
...
function updateResource($request_values) {
    global $conn, $errors, $r_title, $r_iconname, $res_id, $topics_id, $r_info, $source, $link_ids, $link_names, $link_urls, $subpage_show, $rlevels, $tags,$res_id,$mediatype;
    //Start of Non-Ajax File upload
    if (isset($request_values['r_icon'])) {
        ImageIconUpload ();
        }
    //End of Non-Ajax File upload
    $res_id = esc($request_values['resource_id']);
    $r_iconname = esc($request_values['r_img']);
    $r_title = esc($request_values['r_title']);
    $r_info = htmlentities(esc($request_values['r_info']));
    $source = htmlentities(esc($request_values['source']));
    $link_names = $request_values["link_name"];
     $link_urls = $request_values["link_url"];
    if (isset($request_values["link_ids"])) {
        $link_ids = $request_values["link_ids"];
    }
      if (isset($request_values["restypes"])) {
        $restypes = $request_values["restypes"];
    }
...
/*
    ...
    */
    ...
    // Update resource if there are no errors in the form
     if (count($errors) == 0) {
         ...
         $r_info = filter_var ($r_info, FILTER_SANITIZE_STRING);
         $source = filter_var($source, FILTER_SANITIZE_STRING);
         $res_id = filter_var($res_id, FILTER_SANITIZE_NUMBER_INT);
      //   $subpage_show = filter_var($subpage_show, FILTER_SANITIZE_BOOLEAN); 
   $r_sql = "UPDATE resources SET title='$r_title', icon='$r_iconname', info='$r_info', source='$source', multilink=$multilink, subpage_show=$subpage_show, updated_at=now() WHERE id=$res_id";
        ...
        if ($r_updated) { // if the resource is successfully updated
            ...
            // update relationship between resource and tags
            for ($i = 0; $i < count($tags); $i++) {
                $tag = filter_var(esc($tags[$i]), FILTER_SANITIZE_STRING);
                $tg_sql = "INSERT IGNORE INTO resource_tags (resource_id, tag_id) VALUES($res_id, $tag)";
                mysqli_query($conn, $tg_sql);
            }
            ...
            foreach ($restags as $restag) {
                if (!multi_in_array($restag['id'], $tags)){
                    $tag = $restag['id'];
                    $del_tag="DELETE FROM resource_tags WHERE resource_id=$res_id AND tag_id = $tag " ;
                     mysqli_query($conn, $del_tag);
                }
            } 
 
            // update relationship between resource and topic
            for ($i = 0; $i < count($topics_id); $i++) {
                $topic_id = filter_var(esc($topics_id[$i]), FILTER_SANITIZE_NUMBER_INT);
                $t_sql = "INSERT IGNORE INTO resource_topic (resource_id, topic_id) VALUES($res_id, $topic_id)";
                mysqli_query($conn, $t_sql);
                //	if(mysqli_query($conn, $t_sql)){ $_SESSION['message'] = "Topic saved successfully";} else {array_push($errors, "Topic(s) failed to save.");}
            }
            
            $r_topics = getResourcesTopicsSub($res_id); // Get the all the topics from database for the selected resource            
            // Delete any topic that is not associated with the resource
            foreach ($r_topics as $r_topic) {
                if (!multi_in_array($r_topic['id'], $topics_id)){
                    $topic = $r_topic['id'];
                    $del_topic="DELETE FROM resource_topic WHERE resource_id=$res_id AND topic_id = $topic " ;
                     mysqli_query($conn, $del_topic);
                }
            } 
            
            // update relationship between resource and link
                $rlinks = getResourceLink($res_id);  // Get the resource existin links in the database
                for ($j = 0; $j < count($link_urls); $j++) { 
                $link_url = filter_var(esc($link_urls[$j]), FILTER_SANITIZE_URL);  // Snaitize the url input
                $link_name = filter_var(esc($link_names[$j]), FILTER_SANITIZE_STRING) ;
            //    if(isset($request_values["link_ids"][$j])) {$link_id = filter_var(esc($request_values["link_ids"][$j]), FILTER_SANITIZE_NUMBER_INT);}
                 if (!empty($link_name)){   //If the link name is not empty, use the name entered
                    $link_name=$link_name;
                }else{ // else if not link name is not entered, use link.
                  $link_name = 'Link';
                }
                LinkUrlExt($link_url); // Get the $mediatype of the url link
                ...
                if (isset($request_values["link_ids"][$j]) && multi_in_array($request_values["link_ids"][$j], array_column($rlinks, 'id'))) { // Can you find the id of the link among the database link records, if yes, then update, if no, go ahead and create it?
                   $link_id = filter_var(esc($request_values["link_ids"][$j]), FILTER_SANITIZE_NUMBER_INT);
                    $l_sql = "UPDATE links SET url='$link_url', name='$link_name', mediatype_id=$mediatype, modalview=$modalview, updated_at= now() WHERE id = $link_id";
                } else{
                $l_sql = "INSERT INTO links (resources_id, url, name, mediatype_id, modalview, created_at, updated_at) VALUES($res_id, '$link_url', '$link_name', $mediatype, $modalview, now(), now())";
                }
             mysqli_query($conn, $l_sql);
            } 
          //  $rlinks="";
      ...
      //      $rlinks = getResourceLink($res_id);
            foreach ($rlinks as $rlink) {
                if (!multi_in_array($rlink['id'], $link_ids)){
                    $link = $rlink['id'];
                    $del_link="DELETE FROM links WHERE id =$link" ;
            ...
            foreach ($restypes_all as $rtype) { // For each resource types
                $type = filter_var($rtype['id'], FILTER_SANITIZE_NUMBER_INT);      // get the id of the type
           ...
           $rlevels_all = getSchlLevel(); // Get all resource levels
            foreach ($rlevels_all as $rlevel) { // For each resource levels
                $level = filter_var($rlevel['id'], FILTER_SANITIZE_NUMBER_INT);      // get the id of the level
