...
function ResLinkUrl($res_id,$link_urls){
    global $conn,$errors,$request_values,$mediatype;
...
function createResource($request_values) {
   global $conn, $errors, $r_title, $r_iconname, $res_id, $topics_id, $r_info, $source, $restypes, $link_names, $link_urls, $subpage_show, $rlevels, $tags,$link_ids;
    ...
    // create resource if there are no errors in the form
     if (count($errors) == 0) {
      ...
      //   $subpage_show = filter_var($subpage_show, FILTER_SANITIZE_BOOLEAN); 
        ...
        if ($r_inserted) {
            ResTagsSave ($res_id, $tags);  // create relationship between resource and tags         
            ResTopicsSave($res_id, $topics_id);   // create relationship between resource and topic         
            ResLinkUrl($res_id,$link_urls);    // create relationship between resource and link       
            ResTypesSave ($res_id, $restypes);    // create relationship between resource and resource type
            ResLevelsSave ($res_id, $rlevels);  // create relationship between resource and resource levels
